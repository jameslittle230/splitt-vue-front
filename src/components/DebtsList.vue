<template>
    <div v-if="this.$store.state.currentGroup">
        <h2>Debts List (You owe <MoneyDisplay v-bind:amount="debtTotal" />)</h2>
        <ul>
            <li v-for="debt in debts" v-bind:key="debt.id">
                You owe {{ groupMembers[txns[debt.transaction].creator].name }}
                <strong><MoneyDisplay v-bind:amount="debt.amount" /></strong> for
                {{txns[debt.transaction].description}} 
                <span v-if="debt.percentage != 100">({{debt.percentage}}%)</span>
    ...
    computed: {
        debts: function() {
            ...
            return this.$store.state.currentGroup.transactions
                ...
                .map(txn => txn.splits
                    .filter(split => split.debtor == myId)[0]
                )
        },
